...
conda_ism_core_path=$(find . -type f -name "alethic-ism-core*.tar.bz2")
conda_ism_core_path=$(basename $conda_ism_core_path)

conda_ism_db_path=$(find . -type f -name "alethic-ism-db*.tar.bz2")
...
docker build -t krasaee/alethic-ism-api:latest \
 --build-arg CONDA_ISM_CORE_PATH=$conda_ism_core_path \
 --build-arg CONDA_ISM_DB_PATH=$conda_ism_db_path \
 --build-arg GITHUB_REPO_URL=https://github.com/quantumwake/alethic-ism-api.git --no-cache .
